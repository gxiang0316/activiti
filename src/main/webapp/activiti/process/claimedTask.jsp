...
<script type="text/javascript">
		...
		function getData(data){
			...
			for(var i=0; i<data.rows.length; i++){
				rows.push({
					...
					opt: "[<a href=\"#\" onclick=\"add('审批','leaveController.do?taskCompletePageSelect&jspPage="+data.rows[i].description+"&processInstanceId="+data.rows[i].processInstanceId+"&taskId="+data.rows[i].taskId+"','claimedTask')\">审批</a>]"
				});
			}
		...
		$('#claimedTask').datagrid({
	        ...
	        fitColumns:true,
	        showFooter:true,
	        url:'actTaskController.do?claimedTaskDataGrid',  

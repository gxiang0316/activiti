...
function readProcessResouce(processDefinitionId,resourceType) {
	...
	if(resourceType == "xml"){
		title = "查看流程文件";
		url = "actProcessController.do?resourceRead&processDefinitionId="+processDefinitionId+"&resourceType=xml&isIframe"
		//url = "activitiController.do?resourceRead&processDefinitionId=vacation:1:10&resourceType=image&isIframe"
	}

	if(resourceType == "image"){
		title = "查看流程图片";
		url = "actProcessController.do?resourceRead&processDefinitionId="+processDefinitionId+"&resourceType=image&isIframe"
	}
	addOneTab(title, url);
}
...
function getData(data){
	...
	for(var i=0; i<data.rows.length; i++){
		rows.push({
			...
			opt: "[<a href=\"#\" onclick=\"delObj('actProcessController.do?del&deploymentId="+data.rows[i].deploymentId+"','processList')\">删除流程</a>]"
		});
	}
...
$('#processList').datagrid({
    ...
    fitColumns:true,
    showFooter:true,
    url:'actProcessController.do?datagrid',  

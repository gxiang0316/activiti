...
<script type="text/javascript">
		...
		function claimTask(taskId){
			confirm('actTaskController.do?claimTask&taskId='+taskId,'确定认领吗？','waitingClaimTask');
		}
	    // 编辑初始化数据
		function getData(data){
			...
			for(var i=0; i<data.rows.length; i++){
				rows.push({
					...
					opt: "[<a href=\"#\" onclick=\"claimTask('"+data.rows[i].taskId+"')\">认领</a>]"
				});
			}
		...
		$('#waitingClaimTask').datagrid({
	        ...
	        fitColumns:true,
	        showFooter:true,
	        url:'actTaskController.do?unClaimTaskDataGrid',  

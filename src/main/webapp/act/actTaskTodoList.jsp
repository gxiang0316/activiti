	...
	<script type="text/javascript">
		...
		function claim(taskId) {
			$.get('${ctx}/act/task/claim' ,{taskId: taskId}, function(data) {
				if (data == 'true'){
		        	top.$.jBox.tip('签收完成');
		            location = 'actTaskController.do?todo';
				}else{
		        	top.$.jBox.tip('签收失败');
				}
		    });
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="actTaskController.do?todo">待办任务</a></li>
		<li><a href="actTaskController.do?historic/">已办任务</a></li>
		<li><a href="actTaskController.do?process">新建任务</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="act" action="actTaskController.do?todo" method="get" class="breadcrumb form-search">
		<div>
			<label>流程类型：&nbsp;</label>
				...
				<tr>
					<td>
						<c:if test="${empty task.assignee}">
							<a href="javascript:claim('${task.id}');" title="签收任务">11</a>
						</c:if>
					...
					<td>
						...
						<c:if test="${not empty task.assignee}"><%--
							...
							<a href="${ctx}/act/task/form?taskId=${task.id}&taskName=${task.name}&taskDefKey=${task.taskDefinitionKey}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}">任务办理</a>
						</c:if>
